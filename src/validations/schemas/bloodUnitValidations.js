...
export const createBloodUnitSchema = Joi.object({
  ...
  blood_group: Joi.string().valid("A", "B", "AB", "O").required(),

  rh_type: Joi.string().valid("+", "-").required(),

  collection_date: Joi.date().required(),

  volume_ml: Joi.number().min(1).required(),
  ...
  components: Joi.array()
    ...
    .items(
      Joi.object({
        ...
        volume_ml: Joi.number().integer().min(1).required(),
...
export const createDonorWithBloodUnitSchema = Joi.object({
  ...
  age: Joi.number().integer().min(18).max(65).allow(null).messages({
  ...
  hemoglobin: Joi.number().positive().allow(null).messages({
    "number.base": "Invalid hemoglobin.",
    "number.positive": "Hemoglobin must be greater than 0.",
  }),

  last_donation_date: Joi.date().allow(null).messages({
    "date.base": "Invalid last donation date.",
  }),

  collection_date: Joi.date().required().messages({
    "any.required": "Collection date is required.",
    "date.base": "Invalid collection date.",
  }),

  volume_ml: Joi.number().integer().positive().required().messages({
  ...
  remarks: Joi.string().trim().allow("", null).messages({
